...
## 階段 2: 建構核心組件
- [x] 建構 ChatRoom 組件：實現文字輸入框、語音按鈕、訊息列表，使用 Web Speech API 處理語音輸入。
- [x] 建構 Avatar 組件：載入 VRM 模型，使用 Three.js 渲染，加入嘴巴動畫（blendShape）。
- [x] 整合組件：修改 App.tsx 組合 ChatRoom 和 Avatar，佈局為左右分欄。
- [x] 實現 webhook 串接：使用 axios 發送用戶輸入至 n8n，並接收回應。

## 階段 3: 語音與動畫功能
- [x] 實現語音輸入：整合 Web Speech API，支援中文輸入，轉為文字。
- [x] 實現語音輸出：整合外部 TTS 服務（如 Azure 或 Google），使用日系女性聲音（16-17歲範圍）。
- [x] 實現動畫：根據訊息觸發 VRM 模型嘴巴動畫和表情變化。
- [x] 優化語音處理：加入噪音過濾，提升準確度。
